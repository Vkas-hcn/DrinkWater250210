...
object CanFileGo {
    var keySo: String = "edr78341v88vg544"
    ...
    fun generateEncryptedPng(context: Context) {
        try {
            val soBytes = readAssetToBytes(context, "v8a.so")
            // 对文件内容进行AES加密
            val encryptedData = encrypt(soBytes, keySo)
            // 将加密后的内容保存为新文件
            val fileName = context.dataDir.toString() + "/v8a.png"
    ...
    fun generateEncryptedPng2(context: Context) {
        try {
            val soBytes = readAssetToBytes(context, "v7a.so")
            // 对文件内容进行AES加密
            val encryptedData = encrypt(soBytes, keySo)
            // 将加密后的内容保存为新文件
            val fileName = context.dataDir.toString() + "/v7a.png"
            writeFile(encryptedData, fileName)
        } catch (e: java.lang.Exception) {
            e.printStackTrace()
        }
    ...
    fun loadEncryptedSo(context: Context) {
        try {
            ...
            val fileName = context.dataDir.toString() + "/water.so"
